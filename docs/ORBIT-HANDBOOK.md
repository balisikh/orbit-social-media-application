...
The application is engineered to run locally for development, and to deploy to production using **Vercel** + **Supabase** public environment variables for client-safe configuration.
...
- **Authentication:** sign up and sign in (Supabase email/password when configured).
...
2. User creates an account (Supabase).
3. User completes profile basics on `/me/edit`.
...
2. Supabase sends reset email (requires email provider configuration in Supabase).
...
**Important deployment note shown in the screenshot:** ensure `NEXT_PUBLIC_SUPABASE_URL` is your real Supabase URL (not a placeholder), and add `NEXT_PUBLIC_SUPABASE_ANON_KEY` as a second variable (not visible in the screenshot, but required by the app).
...
- **What it is:** Email/password authentication via Supabase when configured.
- **Why it exists:** Secure accounts and server-backed personalization.
- **How it is used:** Users sign up/sign in; sessions are maintained for app routes.
...
- **Cause:** Supabase public environment variables not available in the deployment environment (`NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_ANON_KEY`).
- **Mitigation:** set both variables in Vercel for Production and redeploy; configure Supabase Auth redirect URLs; configure email delivery in Supabase for actual reset emails.

### Issue 2 — “Sign-in is not available on this server.”

- **Symptom:** login cannot proceed in production.
- **Cause:** same as Issue 1: missing Supabase public configuration in production; local preview sign-in is intentionally limited to development environments.
- **Mitigation:** configure Supabase env vars on Vercel + redeploy; verify Supabase Auth settings.
...
1. **Supabase dashboard → API keys** (blur secrets in PDF if sharing externally)
2. **Supabase → Authentication → URL configuration** showing redirect URLs
3. **Orbit live site** `/feed`, `/reels`, `/messages`, `/me`, `/me/edit`, `/auth/login`
4. **Vercel deployment success** page showing “Ready” and the live domain

---

## 10) Conclusion — what to do next time (process improvements)

1. **Pre-deploy checklist** pinned in the repo README or team wiki:
